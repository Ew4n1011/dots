...
while getopts idrstmnh RunStep; do
    case $RunStep in
        ...
        *)
        cat <<EOF
Usage: $0 [options]
            i : [i]nstall dots without configs
            d : install dots [d]efaults without configs --noconfirm
...
if [ $flg_Install -eq 1 ] ||
    ...
    print_log -stat "dots" "It is not recommended to use newly installed or upgraded dots without rebooting the system. Do you want to reboot the system? (y/N)"
    read -r answer
