...
if chk_list "zsh" "${shlList[@]}"; then
    print_log -sec "SHELL" -stat "detected" "zsh"
else
...
if pkg_installed zsh; then
    ...
    if (pkg_installed oh-my-zsh-git || [[ -f "${HOME}/.oh-my-zsh/oh-my-zsh.sh" ]]) && [ "${flg_DryRun}" -ne 1 ]; then
        ...
        sed -i "/^dots_plugins=/c\dots_plugins=(${w_plugin} )${Fix_Completion}" "${Zsh_rc}"
...
if [[ "$(grep "/${USER}:" /etc/passwd | awk -F '/' '{print $NF}')" != "zsh" ]]; then
    ...
    [ "${flg_DryRun}" -eq 1 ] || chsh -s "$(which "zsh")"
else
